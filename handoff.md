...
1. **Write Gen 3 decisions** — `src/data/gen3Decisions.ts` (stub structure exists, needs full authoring)
2. **Write Gen 4 decisions** — `src/data/gen4Decisions.ts`
3. **PWA config** — add `vite-plugin-pwa` to `vite.config.ts`, create manifest, add icons to `/public`
4. **Payment integration** — flip `IS_PREMIUM` in `Legacy.tsx`, wire Stripe or similar
5. **Custom domain** — set up in Firebase Hosting console
6. **Favicon + PWA icons** — add to `/public`
7. **Home page redesign** — current layout is functional but not final
